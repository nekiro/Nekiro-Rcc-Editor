...
const getPreparedImages = () => getImages().map((image) => getPreparedImage(image));

ipcMain.handle("list:images", () => getPreparedImages());

ipcMain.handle("list:replace-image", async (event: Electron.IpcMainInvokeEvent, index: number, path: string) => {
