...
const getAllFiles = async function (dirPath: string, arrayOfFiles: string[] = []) {
...
const execFile = (command: string, args: string[] = [], options: ExecFileOptions) => {
...
export const saveRcc = async (filePath: string | undefined = loadedFilePath) => {
	...
	for (const image of images) {
		data += `<file>${image.path.substring(image.path.indexOf("res.rcc"))}</file>\n`;
...
export const extractToPng = async (directoryPath: string) => {
	...
	for (const image of images) {
		await fs.writeFile(path.join(directoryPath as string, image.fullName), image.data);
	}
